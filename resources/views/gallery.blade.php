    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div id="artworks-grid" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @foreach($artworks as $artwork)
                <div class="group cursor-pointer rounded-lg overflow-hidden shadow-md bg-white" onclick="openArtworkModal({{ $artwork->id }})">
                    <img src="{{ $artwork->image_path ? asset('storage/' . $artwork->image_path) : asset('img/placeholder.jpg') }}" alt="{{ $artwork->title }}" class="w-full h-64 object-cover transition-transform duration-300 group-hover:scale-105">
                    <div class="p-4">
                        <h3 class="font-semibold text-lg text-gray-800">{{ $artwork->title }}</h3>
                        <p class="text-sm text-gray-600">Por: <a href="{{ Auth::check() ? route('profile.show', ['user' => $artwork->user->id]) : route('login') }}" class="hover:underline">{{ $artwork->user->name }}</a></p>
                    </div>
                </div>
            @endforeach
        </div>
